...
pub struct SenderActorRef<Message> {
    sender: Mutex<mpsc::Sender<Message>>, /* it would be nice if we somehow could copy the sender inside Arc instead
        ...
        match self.sender.lock() {
            ...
            Ok(tx) => {
                tx.send(msg).map_err({
                    |err| {
                        match err {
                            mpsc::SendError(msg) => SendError(SendErrorReason::Unreachable, msg),
                        }
                    }
                })
            }
